...
from collie_core.conversation import (default_conversation, conv_templates,
                                   SeparatorStyle)
...
def regenerate(state, request: gr.Request):
    ...
    return (state, state.to_gradio_chatbot(),) + ("", None,) * 3 + (disable_btn,) * 5
...
def add_text(state, text_demo_1, image_demo_1, text_demo_2, image_demo_2, text_3, image_3, request: gr.Request):
    ...
    if args.moderate:
        ...
        if flagged:
            state.skip_next = True
            return (state, state.to_gradio_chatbot(), moderation_msg, None) + ("", None,) * 2 + (disable_btn,) * 5
...
def http_bot(state, model_selector, temperature, max_new_tokens, request: gr.Request):
    ...
    try:
        ...
        response = requests.post(worker_addr + "/worker_generate_stream",
            headers=headers, json=pload, stream=True, timeout=25)
...
title_markdown = ("""
# 🐾 Collie: Multi-Instruct Multimodal AI Assistant
    ...
    with gr.Blocks(title="LLaVA", theme=gr.themes.Base(), css=css) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                textbox_demo_1 = gr.Textbox(label="Demo Text 1 (optional)", show_label=True,
                            placeholder="Enter text and press ENTER").style(container=True)
                imagebox_demo_2 = gr.Image(label="Demo Image 2 (optional)", type="pil")
                textbox_demo_2 = gr.Textbox(label="Demo Text 2 (optional)",show_label=True,
                            placeholder="Enter text and press ENTER").style(container=True)
        ...
        if args.model_list_mode == "once":
            demo.load(load_demo, [url_params], [state, model_selector,
                chatbot, textbox_3, submit_btn, button_row, parameter_row],

...
class AdjustLabelSmoothedCrossEntropyCriterion(_Loss):
    ...
    def forward(self, output, sample, update_num=0, reduce=True):
        ...
        if isinstance(sample, list):
            ...
            loss_v1, sample_size_v1, logging_output_v1 = self.forward(output[0], sample[0], update_num, reduce)
            loss_v2, sample_size_v2, logging_output_v2 = self.forward(output[1], sample[1], update_num, reduce)
        ...
        loss, nll_loss, ntokens = self.compute_loss(net_output, sample, update_num, reduce=reduce)
        sample_size = (
    ...
    def get_lprobs_and_target(self, net_output, sample):
        ...
        lprobs = F.log_softmax(net_output[0], dim=-1, dtype=torch.float32)*conf
    ...
    def compute_loss(self, net_output, sample, update_num, reduce=True):
        lprobs, target, constraint_masks = self.get_lprobs_and_target(net_output, sample)
        if constraint_masks is not None:

...
def n_gram_sampling(tokens,
    ...
    start_position = max(0, np.random.randint(0,
                                              len(tokens) - span_length + 1))
    n_gram_span = tokens[start_position:start_position + span_length]
    return n_gram_span


def short_disorder(tokens,
                   p=[0.9, 0.1, 0, 0,
                      0]):  # untouched + four cases abc, bac, cba, cab, bca
    """
    ...
    while i < l - 1:
        ...
        if permutation != 0 and i == l - 2:
            ...
            outputs[i], outputs[i + 1], outputs[i + 2] = outputs[
                i + 2], outputs[i], outputs[i + 1]
            i += 3
        elif permutation == 4:
            outputs[i], outputs[i + 1], outputs[i + 2] = outputs[
                i + 1], outputs[i + 2], outputs[i]
            i += 3
        else:
            i += 1
    return outputs
...
def long_disorder(tokens, p=0.1, length=20):
    ...
    while i <= len(outputs) - length:
        if np.random.rand() < p:
            outputs[i:i + length //
                    2], outputs[i + length // 2:i +
                                length] = outputs[i + length // 2:i +
                                                  length], outputs[i:i +
                                                                   length // 2]
            i += length
        else:

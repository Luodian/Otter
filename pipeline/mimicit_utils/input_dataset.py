...
class FileDataset(object):
    def __init__(
        ...
        if selected_col_ids is None:
            ...
            self.selected_col_ids = list(range(len(open(self.file_path).readline().rstrip("\n").split(self.separator))))
        else:
        ...
        print("file {} slice_id {} row count {} total row count {}".format(self.file_path, self.slice_id, self.row_count, self.total_row_count))
    ...
    def _init_seek_index(self):
        if self.cached_index:
            ...
            print("local datafile {} slice_id {} use cached row_count and line_idx-to-offset mapping".format(self.file_path, self.slice_id))
        else:
            # make an iteration over the file to get row_count and line_idx-to-offset mapping
            fp = open(self.file_path, "r")
            print("local datafile {} slice_id {} begin to initialize row_count and line_idx-to-offset mapping".format(self.file_path, self.slice_id))
        ...
        print("local datafile {} slice_id {} finished initializing row_count and line_idx-to-offset mapping".format(self.file_path, self.slice_id))
...
if __name__ == "__main__":
    ...
    test_dataset = FileDataset(
        "/mnt/lustre/yhzhang/data/LLaVA-Instruct-150K/complex_reasoning_77k_detail_23k.tsv.head8",
        "0,1,2,3,4,5,6,7",
    )

...
# def process_json(input_json_path, output_json_path, output_img_dict_path, identifier="coco"):
#     with open(input_json_path, "r") as f:
#         data = json.load(f)
...
#     pbar = tqdm(total=len(data))
#     for entry in data:
#         # Extract relevant information from the original dictionary
#         original_id = str(entry["id"]).replace("/", "_")
#         conversations = entry["conversations"]
...
#         for idx, conversation in enumerate(conversations):
#             if conversation["from"] == "human":
#                 # Create keys and instruction dictionary for the MIMIC-IT format
#                 ins_key = f"{identifier}_{original_id}_{counter}_{str(ins_idx).zfill(2)}"
...
#                 if idx == 0 and any([answer_format in question for answer_format in answer_format_template]):
#                     for answer_format in answer_format_template:
#                         if answer_format in question:
#                             ret_answer_format = answer_format
#                             break

#                 if ret_answer_format != "" and idx > 0:
#                     question += f" {ret_answer_format}"
...
#     print(f"Saving the new JSON and image dictionary to the disk..")
#     with open(output_json_path, "w") as f:
#         json.dump(output_data, f)

#     image_df = pd.DataFrame.from_dict(image_dict, orient="index", columns=["base64"])
#     image_df.to_parquet("/home/luodian/projects/Otter/archived/data/LA_MIX_Images.parquet", engine="pyarrow")


if __name__ == "__main__":
    input_json_path = "/raid/bli/data/llava_v1_5_mix665k.json"
    # splits_list = ["gqa", "ocr_vqa", "textvqa", "vg", "text"]
    splits_list = ["text"]

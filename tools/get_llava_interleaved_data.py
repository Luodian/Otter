...
for cur_dict in tqdm(cur_file):
    if "CONV" in target_json_path:
        for cur_round in range(0,len(cur_dict['conversations']),2):
            ...
            instruction = cur_dict['conversations'][cur_round]["value"].strip().replace("<image>","")#.replace("\n","")#.replace("<image>\n","") #cur_meta[uniq_id]["question"]
            answer = cur_dict['conversations'][cur_round+1]["value"].strip().replace("<image>","")#.replace("\n","#").replace('\r','#').replace('\t','#') #cur_meta[uniq_id]["answer"]
        ...
        answer = cur_dict['conversations'][1]["value"].strip().replace("<image>","")#.replace("\n","#").replace('\r','#').replace('\t','#') #cur_meta[uniq_id]["answer"]
